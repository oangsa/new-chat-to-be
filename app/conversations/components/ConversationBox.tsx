...
const ConversationBox: React.FC<ConversationBoxProps> = async ({ 
  data, 
  selected 
}) => {
  const otherUser = await useOtherUser(data);
  const session = await getDataByCookie();
  ...
  const userEmail = useMemo(() => session.username,
  [session.username]);
  ...
  const hasSeen = useMemo(() => {
    ...
    return seenArray
      .filter((user) => user.username === userEmail).length !== 0;
